...
bin/read_hess_nr: out/read_hess_nr.o out/rec_tools_nr.o \
	...
	$(CC) $(LDFLAGS) $(SOEXEFLAGS) \
           ...
           $(HESSIO_LIB) -lm  \
           -o $@
	-(cd `dirname $@` && ln -s read_hess_nr read_simtel_nr)
	-(cd `dirname $@` && ln -s read_hess_nr read_cta_nr)
...
bin/read_hess: out/read_hess.o out/rec_tools.o out/reconstruct.o \
	...
	$(CC) $(LDFLAGS) $(SOEXEFLAGS) \
           ...
           $(HESSIO_LIB) -lm  \
           -o $@
	-(cd `dirname $@` && ln -sf read_hess read_simtel)
	-(cd `dirname $@` && ln -sf read_hess read_cta)
...
bin/extract_hess: out/extract_hess.o lib/$(call lib_expand,hessio)
	$(CC) $(LDFLAGS) $(SOEXEFLAGS) \
           ...
           $(HESSIO_LIB) -lm  \
           -o $@
	-(cd `dirname $@` && ln -s extract_hess extract_simtel)
	-(cd `dirname $@` && ln -s extract_hess extract_cta)
...
include/eventio_version.h: $(LIBHESSIO_SOURCES)
	grep RELEASE $@ | sed 's/ EVENTIO_RELEASE/ EVENTIO_BASE_RELEASE/' > $@.tmp
